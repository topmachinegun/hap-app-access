...
| §5 个人级授权 | Token 获取 + MCP 配置（**强制走 MCP 协议**，官方持续升级）+ 过期刷新 + 官方应用最佳实践（§5.7 发现序列 + 运行时直调） |
| §6 API Host | 明道云/Nocoly/私有部署域名 |
| §7 通用调用规范 | **参数命名以 `tools/list` 为 SSOT**（§7.1 法定导航）、Filter、分页、字段 ID |
| §8 通用陷阱清单 | 14 个高频坑与解法（knowledge_search 排查改为三步走同构） |
| §9 错误码速查 | 10001 vs 600101 区分 + 10001 排错三步走 |
| §10 快速决策流程 | 选型决策树（含官方应用分支） |
| §11 相关技能 | 5 个关联技能索引 |
