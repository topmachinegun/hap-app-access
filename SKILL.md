...
> **30 秒上手**：用 `--app` 参数按应用名直接调用，token 自动管理，无需手动配置。
...
```
personal_mcp:
  - 产品管理 (app_id: abc123...)  [org: 明华创新]
  - ClawCRM  (app_id: def456...)  [org: 明华创新]
app_mcp:
  - ClawCRM  (app_id: def456...)  [appkey: ****]
```

### 1.2 调用工具

```bash
# Personal MCP 应用（token 自动刷新，无需关心）
hap-access call --app 产品管理 --tool get_record_list \
  --args '{"worksheet_id":"...","pageSize":20,"search":"水印"}'

hap-access call --app ClawCRM --tool get_record_list \
  --args '{"worksheet_id":"...","pageSize":50,"search":"华北油田"}'

# 列出应用可用工具
hap-access list-tools --app ClawCRM
```

### 1.3 Token 自动刷新（透明）

智能体**不需要手动管理 token**：
- 调用 `--app` 时底层自动检查 token 有效性
- 过期时自动用 `account + password` 刷新（调 `md-generate-mcp-config`）
- 刷新结果写回 `config/hap-config.local.json`

### 1.4 新应用注册

当需要访问一个**不在配置中**的应用时：

1. `hap-access config --list-apps` 确认不在列表
2. 通过 MCP 发现序列找到 app_id（见 §4.7）
3. 追加到 `config/hap-config.local.json` 的 `personal_mcp.apps`：
   ```python
   import json
   ...
   cfg['personal_mcp']['apps'].append({
       'org_name': '组织名', 'org_id': '...', 'app_name': '新应用', 'app_id': '...'
   })
...
1. **配置文件是唯一事实源**：所有 HAP 连接信息从 `config/hap-config[.local].json` 获取
2. **禁止硬编码**：不得在代码或对话中硬编码 token/appkey/sign/appId
3. **禁止跳过初始化**：local 配置不存在或凭据为空时，必须先完成初始化（§2.3）再执行业务
4. **token 刷新透明化**：使用 `--app` 时自动刷新，不需要手动管理 token
...
从 v1.7 起，skill 随仓库携带统一配置文件，集中管理所有凭据和应用信息。**智能体平台无需人工干预**。
...
```json
{
  ...
  "personal_mcp": {
    ...
    "apps": [
      {"org_name": "...", "org_id": "...", "app_name": "...", "app_id": "..."}
    ]
  },
  "app_mcp": {
    "api_base": "https://api.mingdao.com",
    "apps": [
      {"org_name": "...", "app_name": "...", "app_id": "...", "appkey": "", "sign": ""}
    ]
  }
}
```
...
#### 发现后的访问缺口：OAuth scope ≠ 应用访问权限

发现序列（`get_org_list` → `get_app_list`）能列出你可见的所有应用，但**能发现 ≠ 能访问**。业务工具（`get_app_worksheets_list` / `get_record_list` / `knowledge_search` 等）要求传入 `appId`，服务端会验证当前凭证是否有权访问该 `appId`：

| 凭证类型 | scope 规则 | 跨应用访问 |
|----------|-----------|-----------|
| Personal OAuth（默认 OAuth App） | 通常覆盖用户所在 org 的所有应用 | ✅ 通常可以 |
| Personal OAuth（限定 OAuth App，如 ClawCRM 专用） | 仅覆盖 OAuth 授权时勾选的应用 | ❌ 只能访问授权的那几个 |
| Appkey+Sign | **绑定单个应用**，一个 Appkey 只能访问它对应的应用 | ❌ 严格 1:1 |

**典型 10001 场景**：`get_app_list` 返回了"产品管理"应用，但 `get_app_worksheets_list(appId=产品管理)` 报 10001——因为当前 token/appkey 的 scope 不覆盖该应用。

**解决方案**：为每个需要访问的应用创建**独立 profile**（见 §5.11 "One Profile per App" 原则）。如果 Personal OAuth scope 不足，需要：
1. 用 `hap-oauth-mcp` 重新授权，勾选目标应用
2. 或获取目标应用的 Appkey+Sign（需应用管理员提供），创建 `app_mcp` / `v3_api` profile

> **智能体行为准则**：发现序列成功但业务工具报 10001 时，**不可反复重试**，必须告知用户"当前凭证的 scope 不覆盖目标应用"，引导创建新 profile。
...
## 6. 通用调用规范（MCP + V3 API 共用）

### 6.1 参数命名：以 `tools/list` 为唯一权威来源（SSOT）
...
### 7.10 Personal MCP 的 appId 和 ai_description

应用级 MCP 调用不需要这两个参数。**个人级 MCP 的每次调用必须提供**，否则返回 401。

### 7.11 官方应用不支持 Appkey+Sign
...
### 7.12 Personal MCP 的 get_app_list 参数是 org_id
...
### 7.14 knowledge_search 返回「账号未登录」的排查路径
...
### 7.16 MDAccountLogin 「服务异常」≠ 服务端故障
...
## 附录 A：兼容旧模式（v1.6 Profile / Token Broker）

> **v1.7+ 推荐使用 `--app` 模式**（§1-§3）。以下内容为兼容旧部署保留。

### A.1 Access Profile（v1.6）

Profile 文件位于 `~/.local/share/hap-app-access/profiles/<name>.json`（权限 0600）。
...
CLI 用法：`hap-access call --profile claw-crm --tool ... --args ...`

### A.2 Token Broker 中控服务（systemd）

多业务方共享服务器时，用守护进程统一管理 token 刷新。

```
config.toml (account/password) → hap-token-broker.service → tokens/<profile>.json
```

部署：`sudo bash install.sh` → 编辑 config.toml → `sudo bash install.sh --restart`

消费方：`hap-token get claw-crm` 或直读 `~/.local/share/hap-token-broker/tokens/claw-crm.json`。

### A.3 mcp_token.py（单机开发工具）

```bash
export MINGDAO_ACCOUNT='13800138000'
export MINGDAO_PASSWORD='your_password'
python3 scripts/mcp_token.py            # 打印有效 MCP URL
python3 scripts/mcp_token.py --refresh  # 强制刷新
```

> v1.7+ 不再推荐，已被 `config_loader.ensure_token()` 替代。
...
**v2.0.0 变更**：
- 文档全面重构：以「智能体快速上手」为主线，`--app` 模式作为首选路径
- §1 快速开始 + §2 配置文件 + §3 CLI 参考= 智能体 30 秒可用
- 旧模式（Profile / Token Broker / mcp_token.py）收入附录 A
- 陷阱清单精简合并，去除重复内容
