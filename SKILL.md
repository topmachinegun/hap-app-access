...
#### One Profile per App 原则

每个 profile 对应**一个 HAP 应用**。访问 N 个应用 = 需要 N 个 profile（或一个 scope 覆盖所有应用的 Personal OAuth profile）。

原因：
- `personal_mcp`：token 的 OAuth scope 限定可访问的应用集合；不同 OAuth App 授权范围不同
- `app_mcp` / `v3_api`：Appkey+Sign 是应用管理员发放的，1 组凭证 = 1 个应用，严格绑定

多 profile 示例：
```
~/.local/share/hap-app-access/profiles/
  claw-crm.json          # mode=personal_mcp, token_source=broker:claw-crm
  product-mgmt.json      # mode=personal_mcp, token_source=broker:product-mgmt  (另一个 broker 条目)
  orders.json            # mode=app_mcp, appkey=..., sign=...  (订单应用)
```

或用一个 scope 足够宽的 Personal OAuth profile（默认 OAuth App 生成的 token 通常覆盖用户可见的所有应用）覆盖多应用场景。
...
**v1.7.0 变更**：
- 新增 `config/hap-config.json` 统一配置文件，集中管理所有凭据和应用信息（§5.13）
- 新增 `config_loader.py` 模块，支持从配置文件解析虚拟 profile
- CLI 新增 `--app` 参数，可按应用名直接调用（无需单独 profile 文件）
- CLI 新增 `config --list-apps` / `config --show` 子命令

**v1.6.2 变更**：
- `mcp_client.py` 修复 `url:` token_source 未实现的 bug（§5.11 文档已声明但代码缺失）
- §5.7 补「发现后的访问缺口：OAuth scope ≠ 应用访问权限」——能发现 ≠ 能访问，业务工具 10001 的第二类根因
- §5.11 补「One Profile per App 原则」——显式声明每个 profile 对应一个 HAP 应用
