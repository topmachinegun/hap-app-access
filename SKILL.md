...
| MCP `get_app_list` | 调用 Personal MCP 工具获取全部可见应用列表 | 代码/脚本自动化 |
| V3 API `GET /v3/app/info` | 使用 Appkey+Sign 请求，返回 `data.appId` | 已有应用级凭证 |

> **`get_app_list` 也必须传 `appId`**：虽然此工具返回所有应用列表（逻辑上不依赖特定应用），但 HAP V3 代理层仍要求 `appId` 参数，否则返回 `error_code: 10001`。可传入任意一个已知应用的 `appId`（如当前正在操作的 appId）作为鉴权凭证。
>
> 示例：已知 CRM2025 的 appId，调用 `get_app_list(appId=CRM2025_APP_ID)` 可返回包括其他应用在内的完整列表，从中按名称匹配目标应用的 appId。
...
**OAuth Bearer Token 不能用于 V3 REST API**。V3 API 只认 `HAP-Appkey` + `HAP-Sign` 请求头。如果需要用 V3 API 访问某个应用，必须使用该应用的 Appkey+Sign。
...
### 8.12 Personal MCP 的 get_app_list 必须传 appId

`get_app_list` 返回用户可见的所有应用列表，逻辑上不依赖特定应用，但 **HAP V3 代理层仍要求 `appId` 参数**。不传 `appId` 会返回 `error_code: 10001`（Http Headers verification failed），不是缺少参数的提示，容易误判为域名白名单问题。

```json
// ❌ 错误：不传 appId 返回 10001
{ "ai_description": "list apps" }

// ✅ 正确：传任意已知 appId
{ "appId": "<任意已知appId>", "ai_description": "list apps" }
```

**解法**：调用 `get_app_list` 时，传入任意一个已知应用的 `appId` 作为鉴权凭证即可。
...
| `10001` | HTTP Headers 验证失败 | OAuth token 域名不在白名单 **或 Personal MCP 缺少 appId 参数** | 确认使用 `api.mingdao.com`；Personal MCP 调用必须传 appId |
...
> 1. OAuth token 的域名白名单问题（不是 token 本身问题）
> 2. Personal MCP 调用缺少 `appId` 参数（`get_app_list` 等元数据工具也需要）
>
> 排查顺序：先检查是否漏传 `appId`，再检查域名白名单。
