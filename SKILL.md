...
### 7.1 参数命名：按 Schema 传参

HAP 工具参数命名**混合使用 camelCase 和 snake_case**，不能一概而论：

| 命名风格 | 代表参数 |
|---------|---------|
| camelCase | `pageSize`、`pageIndex`、`useFieldIdAsKey`、`appId`、`responseFormat` |
| snake_case | `org_id`、`worksheet_id`、`row_id`、`view_id` 等**资源 ID 类**；`ai_description`（一线必填法定参数） |

**规则**：调用前先用 MCP `tools/list` 或查 V3 API 文档确认 schema 中的确切参数名，不要想当然统一为一种风格。
...
### 8.14 knowledge_search 在部分 token 下返回「账号未登录」

Personal MCP 的 `knowledge_search` 工具对部分 token / 部分应用返回「账号未登录」，即使其他工具（如 `get_org_list`）正常工作。原因不明，可能与 token 权限范围或知识库配置有关。

**解法**：改走 **`get_record_list` + `filter`** 路径——明道云知识库本质是个工作表应用，其「知识条目」存储在工作表里，可用常规工作表查询逻辑访问：
...
4. `get_record_details(worksheet_id, appId, row_id)` 拿正文（注意正文通常是 **kdocs 外链** 或 **附件**，非纯文本）
